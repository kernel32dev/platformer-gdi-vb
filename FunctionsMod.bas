...
End If
    ...
    X = Asc(Mid$(Text, Z, 1))
        ...
        UITexture.RenderTo Hdc, DestX, DestY, 8, H, (X - 48) * 8, Y
        ElseIf X = 64 Then '@ = Coin
        UITexture.RenderTo Hdc, DestX, DestY, 8, 8, 0, 40
        ElseIf X = 120 Then 'x = X
        UITexture.RenderTo Hdc, DestX, DestY, 8, 8, 32, 40
        ElseIf X = 84 Then 'T = Time
        UITexture.RenderTo Hdc, DestX, DestY, 24, 8, 8, 40
        DestX = DestX + 16
        ElseIf X = 77 Then 'M = Mario
        UITexture.RenderTo Hdc, DestX, DestY, 40, 8, 0, 32
        DestX = DestX + 32
        ElseIf X = 76 Then 'L = Luigi
        UITexture.RenderTo Hdc, DestX, DestY, 40, 8, 40, 32
        DestX = DestX + 32
        ElseIf X = 73 Then 'I = Infinity
        UITexture.RenderTo Hdc, DestX, DestY, 16, 8, 40, 40
        DestX = DestX + 8
        End If
    DestX = DestX + 8
    Next
...
If Icon >= 0 Then
    With BPs(Icon)
        ...
        'Else
        GridTextureBuffer.RenderTo Hdc, X, Y, PPB, PPB, _
        .TextureX * PPB, .TextureY * PPB
        'End If
    End With
Else
    With EPs(Not Icon)
    EntitiesTexture.RenderTo Hdc, X, Y, PPB, PPB, _
...
SrcBuff.RenderTo DestBuff.Hdc, X, Y, SrcW, SrcH, SrcX, SrcY
Else
...
Public Function HitQuestionBlock(ByVal X As Long, ByVal Y As Long, Optional ByVal Direction As Long)
With Entities.AddEntity(Grid)
Grid.SetBlock X, Y, 8
.EntityType = PlayerSpawner 'Aka Block being hit
.PosX = X
.PosY = Y
.MY = -0.1
.GY = 0.01
